...
def main():
    parser = argparse.ArgumentParser(
        prog="transapt-run",
        description="TransApt: Generate nucleic acid sequences from a PDB structure file",
    )
...
def analyze():
    """transapt-analyze: Generate sequences, extract shape features, and cluster."""
    parser = argparse.ArgumentParser(
        prog="transapt-analyze",
        description="TransApt: Generate → Shape features → Clustering → Filter",
    )
    parser.add_argument("--input", "-i", required=True,
                        help="Input PDB file or directory containing .pdb files")
    parser.add_argument("--output", "-o", required=True,
